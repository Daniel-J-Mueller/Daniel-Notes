...
As long as the final model is not heavily inferencing, and is only merging with light inferencing to handle discrepencies or subtle novel continuations, the model need not be trained overly extensively. The throughput does not require the same compute as first-pass inferencing.
